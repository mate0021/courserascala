...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    wordOccurrences(s.mkString.flatMap(x => List(x)))
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def aux(let: List[(Char, Int)]): List[Occurrences] = {
      let match {
        case List() => List(List())
        case l :: ls =>
          val sub = aux(ls)
  ...
  def combine(letters: List[Char]): Set[List[Char]] = { 

    def aux(let: List[Char]): Set[List[Char]] = {
      let match {
        case List() => Set(List())
        case l :: ls =>
          val sub = aux(ls)
